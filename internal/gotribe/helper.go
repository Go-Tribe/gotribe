...
const (
	// recommendedHomeDir 定义放置 gotribe 服务配置的默认目录.
	recommendedHomeDir = ".gotribe"

	// defaultConfigName 指定了 gotribe 服务的默认配置文件名.
	defaultConfigName = "gotribe.yaml"
)
...
func initConfig() {
	...
	// 读取环境变量的前缀为 GoTribe，如果是 gotribe，将自动转变为大写。
...
// initStore 读取 db 配置，创建 gorm.DB 实例，并初始化 gotribe store 层.
