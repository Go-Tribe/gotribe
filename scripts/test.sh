...
mb::test::login()
{
  ${CCURL} "${Header}" http://${INSECURE_SERVER}/login \
    -d'{"username":"root","password":"gotribe1234"}' | grep -Po 'token[" :]+\K[^"]+'
}
...
mb::test::user()
{
  ...
  ${CCURL} "${Header}" "${token}" http://${INSECURE_SERVER}/v1/users \
    -d'{"username":"colin","password":"gotribe1234","nickname":"belm","email":"my@dengmengmian.com","phone":"1818888xxxx"}' > /dev/null
...
mb::test::post()
{
  ...
  ${CCURL} "${Header}" "${token}" http://${INSECURE_SERVER}/v1/users \
    -d'{"username":"posttest","password":"gotribe1234","nickname":"belm","email":"my@dengmengmian.com","phone":"1818888xxxx"}' > /dev/null

  # 2. posttest 登录 gotribe
  tokenStr=`${CCURL} "${Header}" http://${INSECURE_SERVER}/login -d'{"username":"posttest","password":"gotribe1234"}' | grep -Po 'token[" :]+\K[^"]+'`
