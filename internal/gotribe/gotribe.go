...
// NewGoTribeCommand 创建一个 *cobra.Command 对象. 之后，可以使用 Command 对象的 Execute 方法来启动应用程序.
...
Find more gotribe information at:
	...
	cmd.PersistentFlags().StringVarP(&cfgFile, "config", "c", "", "The path to the gotribe configuration file. Empty string for no configuration file.")
...
//	pb.RegisterGoTribeServer(grpcsrv, user.New(store.S, nil))
//
